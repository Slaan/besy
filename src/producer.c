  ...
  int PID = *(int*) pid;
      ...
      pthread_mutex_unlock(&prod1_mutex);	
			...
			while(p_rb->p_in == p_rb->p_out && p_rb->count == MAX_BUFFER_SIZE) {
				printf("Producer 1 (%d): Buffer ist voll. Bin ratzen.\n", PID);
	  	...
	  	*(p_rb->p_in) = (char) z_var;
		  ...
		  (p_rb->count)++;
		  if(p_rb->count != 0) {
		  	printf("Producer 1 (%d): Buffer nicht mehr voll.\n", PID);
...
void* producer2(void* pid) {
	int z_var = 0x40;
  int PID = *(int*) pid;
		printf("Producer 2 (%d): starte prod 2\n", PID);
		while(true) {
      ...
      pthread_mutex_unlock(&prod2_mutex);	
			...
			pthread_mutex_lock(&rb_mutex);
			while(p_rb->p_in == p_rb->p_out && p_rb->count == MAX_BUFFER_SIZE) {
				printf("Producer 2 (%d): Buffer ist voll. Bin ratzen.\n", PID);
				pthread_cond_wait(&is_not_full, &rb_mutex);
	  	...
	  	*(p_rb->p_in) = (char) z_var;
		  ...
		  (p_rb->count)++;
		  if(p_rb->count != 0) {
			  printf("Producer 2 (%d): Buffer nicht mehr voll.\n", PID);
			  pthread_cond_signal(&is_not_empty);
		  }
		  pthread_mutex_unlock(&rb_mutex);
		  sleep(3);
